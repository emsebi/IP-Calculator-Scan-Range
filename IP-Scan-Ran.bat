...
TITLE Ip Scanner EMSebi
:: IP Scaner Project Start 1400 - 1402-12-12
:: Reversion 2
:: Change Scan Range Subnet CALCULATING For Old Version 
:: FOR /L %i IN (1,1,254) DO ping -n 1 172.16.16.%i | FIND /i "Reply">>c:\ipaddresses.txt
:: ########################################################################################
:: ###################################### -=EMSebi=- ######################################
:: ########################################################################################
...
#set LOGF=SCAN__%_MYIP%.txt
#set LOGF=SCAN-IP.txt
...
set LOGF=SCAN_%_IPNET%.txt
echo Scaner IP Range By EMSebi   >%LOGF% & echo --------------------------------------- >>%LOGF%
echo Start Scan  %date% - %time%: >>%LOGF% & echo --------------------------------------- >>%LOGF%

set /p _NETMASK=Netmask CIDR format (0 - 32): 
if "!_NETMASK!"=="" (
	Color 0C
    echo\ & echo Empty - Empty Subnet ""Error "" Enter Subnet Number 0-32
